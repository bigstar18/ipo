...
public class GetBallotNoUtils {
	...
	public  void executeApplyBallotNo() {
		...
		// ========1. 获取每个商品的中签率=====（此次发布的商品数量/当前申购所有商品单位数*1000）successRateMap<商品编号,中签率>
		...
		// -----------------------------------根据系统时间T-2获取配号信息（高波）后期修改优化ipoDistrMapper.selectAll() 需要根据时间-2天查询记录-----------------
        //查询配号表中所有配号信息
        ...
        if(ipoBallotNoInfoList != null || ipoBallotNoInfoList.size() > 0){
        	for(int i =0;i < ipoBallotNoInfoList.size();i++){
        		...
        		logger.info("===5. 将中签信息统一入库=="+"商品编号："+ipobal.getCommodityid()+"中签号："+ipobal.getBallotno()+",中签起始位置："+ipobal.getBallotnostartlen()+"中签结束为位置："+ipobal.getBallotnoendlen());
        		//调用入库方法------------------------------高波提供
        ...
        ipoOrderMapper.deleatAll();

        ipoNumberOfrecordsMapper.insertAll();
        ipoNumberOfrecordsMapper.deleatAll();
	 ...
	 * 
	 * @param commdityCorreSuccessNo<商品编号，中签号码>
	 * @param ipoDistrList 配号信息表中所有记录
	 */
	public void calculateCommdityOfUserSuccessNoCount(Map<String, String> commdityCorreSuccessNo,
	...
	public void getBallotNoCount(IpoDistribution ipoDis,String ballotNo){
		...
		logger.info("ipoDistrMapper.updateBycomAndUserid(ipoDis)，根据用户编号，商品编号作为更新条件去更新中签数量,"+"中签号："+ballotNo+"，用户编号："+ipoDis.getUserid()+"，商品编号："+ipoDis.getCommodityid()
		+"，配号起始号："+ipoDis.getStartnumber()+"，配号数量："+ipoDis.getPcounts()+"，中签数量："+ipoDis.getZcounts());
	}
     ...
     * @param ballotNo
     */
//	public void getBallotNoCount(List<IpoDistribution> ipoDistrList, String ballotNo) {
		// 形成一个新的配号列表
