...
public interface IpoCommodityConfMapper {
	...
	// 查询待上市的商品
	List<IpoCommodityConf> queryListingCommodity(
			@Param("param") IpoCommodityConf example);

	// hxx 偷懒
	int querySubjectCommoCount(@Param("subject") String subject);

	// hxx
	int addFeeSubject(@Param("parentSubject") String parentSubject,
			@Param("commoId") String commoId);
