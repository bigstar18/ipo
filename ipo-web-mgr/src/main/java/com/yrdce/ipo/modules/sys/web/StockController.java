...
public class StockController {
	...
	@ResponseBody
	public String transferRecordList(@RequestParam("page") String page,
			@RequestParam("rows") String rows, StockTransferrecords example)
			throws IOException {
	...
	@ResponseBody
	public String freezeRecordList(@RequestParam("page") String page,
			@RequestParam("rows") String rows, StockFreeze example)
			throws IOException {
		log.info("查询过户记录列表" + example.toString());
		log.debug(example.toString());
		try {
			// StockTransferrecords example = new StockTransferrecords();
			List<StockFreeze> tlist = stockFreezeService.freezeRecord(page,
					rows, example);
	...
	public String openTransfer(HttpServletRequest request, Model model)
		...
		StockTransferrecords stockTransferrecords = stockTransferrecordsService
	...
	@ResponseBody
	public String addNewTransfer(StockTransferrecords example,
			HttpSession session) throws IOException {
		log.info("增加过户");
		try {
			log.debug(example.toString());
			Long quatity = Long.parseLong(example.getTransferNumber()
					.toString());
			...
			// 验证客户持仓
			if (stockTransferrecordsService.checkCommQuatity(quatity, firmid,
					commid) == false) {
				return "noquatity";
			}
			// 更新持仓量
			customerHoldSumService.myfreezeCustomerHold(quatity, firmid,
					commid, bsFlag);
			// 增加记录
	...
	public String deleteInfo(
		...
		for (int i = 0; i < Ids.length; i++) {
			StockTransferrecords example = stockTransferrecordsService
					.findById(Long.parseLong(Ids[i]));
			// log.info("测试:" +
			// example.getTransferNumber().longValue()+example.getCustomeridApply()
			// + "00"+example.getCommodityid().toString());
			if (example.getState() == 0) {
				customerHoldSumService.unfreezeCustomerHold(example
						.getTransferNumber().longValue(),
						example.getCustomeridApply() + "00", example
								.getCommodityid().toString(), (short) 1);
			}
		}
	...
	@ResponseBody
	public String unfreezeInfo(
			@RequestParam("applicationIds") String applicationIds)
			throws IOException {
		log.info("批量解冻信息:" + applicationIds);
		String[] Ids = applicationIds.split(",");
		for (int i = 0; i < Ids.length; i++) {
			StockFreeze example = stockFreezeService.findById(Long
					.parseLong(Ids[i]));
			// log.info("测试:" +
			// example.getTransferNumber().longValue()+example.getCustomeridApply()
			// + "00"+example.getCommodityid().toString());
			...
			if (example.getState() == 0) {
				customerHoldSumService.unfreezeCustomerHold(example
						.getFreezeNumber().longValue(), example.getCustomerid()
						+ "00", example.getCommodityid().toString(), (short) 1);
	...
	public String findAllStorages(@RequestParam("page") String page,
		...
		try {
			List<StockTransferrecords> tlist = stockTransferrecordsService
					.selectByPage(page, rows, storage);
	...
	public String update(HttpServletRequest request,
			StockTransferrecords stockTransferrecords) throws IOException {
		try {
			stockTransferrecords.setOperator(getloginUserId(request));
			StockTransferrecords example = stockTransferrecordsService
					.findById(Long.parseLong(stockTransferrecords
							.getApplicationId().toString()));
			// log.info("测试:" +
			// example.getTransferNumber().longValue()+example.getCustomeridApply()
			// + "00"+example.getCommodityid().toString());
			if (stockTransferrecords.getState() == 2) {
				customerHoldSumService.unfreezeCustomerHold(example
						.getTransferNumber().longValue(),
						example.getCustomeridApply() + "00", example
								.getCommodityid().toString(), (short) 1);
			}
			if (stockTransferrecords.getState() == 1) {
				customerHoldSumService.reduceCustomerHold(example
						.getTransferNumber().longValue(),
						example.getCustomeridApply() + "00", example
								.getCommodityid().toString(), (short) 1);
				customerHoldSumService.increaseCustomerHold(example
						.getTransferNumber().longValue(),
						example.getCustomeridAccept() + "00", example
								.getCommodityid().toString(), (short) 1);
			}
