...
<script src="<%=request.getContextPath()%>/static/jquery-easyui/jquery.easyui.min.js"  type="text/javascript"></script>
  ...
  $(document).ready(function() {
	    $('#dg').datagrid({
	      url: '<%=request.getContextPath()%>/chargeRoleController/query?t='+Math.random(), //从远程站点请求数据的 URL。
	      method:"post",
	      height:430,
	  ...
	  function add(){
		  var url_='<%=request.getContextPath()%>/chargeRoleController/add?t='+Math.random();
		  window.open(url_);
	  };
	  
	  function edit(id){
		  var url_='<%=request.getContextPath()%>/chargeRoleController/edit?id='+id+'&t='+Math.random();
		  window.open(url_);
	  }
