...
<script src="<%=request.getContextPath()%>/static/jquery-easyui/jquery.easyui.min.js"  type="text/javascript"></script>
  ...
  $(document).ready(function() {
	    var url_='<%=request.getContextPath()%>/trusteeshipCommodityController/queryApply?states='+states+'&t='+Math.random();
	    $('#dg').datagrid({
	      ...
	      columns: [
	        [
	          ...
	          {field: 'oper',title: '预减持设置',width : 200, align: 'center',
	              formatter: function(value, row, index) {
	            	  if(row.state=="10"){
	            		   return "已设置";
	            	  }else if(row.state=="9"){
	            		  return "<a href=\"#\" onclick=\"add("+row.id+")\">" + "未设置" + "</a>";
	            	  }
	            	  return "";
	               }
	          },
	 ...
	 function add(id){
		  var url_='<%=request.getContextPath()%>/trusteeshipCommodityController/addPrereduce?id='+id;
